...
class ClassifierLoss:
    def __init__(self, alpha, gamma):
        ...
        self.focal_loss = FocalLoss(weight=weight, gamma=gamma, reduction="mean")
    ...
    def compute_loss(
        ...
        table_loss = self.compute_batch_loss(
            batch_table_name_cls_logits, batch_table_labels, batch_size
        )
        column_loss = self.compute_batch_loss(
            batch_column_info_cls_logits, batch_column_labels, batch_size
        )
