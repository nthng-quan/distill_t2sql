...
class SinusoidalLR(_LRScheduler):
    """Sets the learning rate of each parameter group to follow a sinusoidal schedule between base_lr and eta_min.

    .. warning::
        It is recommended to call :func:`.step()` for :class:`SinusoidalLR`
        after each iteration as calling it after each epoch may not yield the intended sinusoidal schedule.
    """
    ...
    def get_lr(self) -> List[float]:
        """Compute learning rate using sinusoidal form of the scheduler."""
        if not self._get_lr_called_within_step:
            warnings.warn(
                "To get the last learning rate computed by the scheduler, "
                "please use `get_last_lr()`.",
        ...
        return [
            ...
            * (
                1
                # - math.cos(self.step_size * math.pi * self.last_epoch / self.max_epochs)
                - math.cos(
                    math.pi
                    * (self.last_epoch % (self.max_epochs // 2))
                    / (self.max_epochs // 2)
                )
            )
            for base_lr in self.base_lrs
